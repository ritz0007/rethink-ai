# Building Metacognition — Header Design

> Rethink AI — Phase 15, Guide 3 of 5
...
src/
  meta/
    meta.h    ← Structures + API
    meta.c    ← Implementation
```
...
### CognitiveStrategy — A way of thinking
...
### PerformanceRecord — What happened?
...
typedef struct {
    ...
    /* Meta-learning */
    float base_learning_rate;     /* α_base */
    float effective_learning_rate; /* α_effective (adapted) */
    float learning_trajectory;    /* Performance trend (+improving, -declining) */
...
```
  ...
  │                     RETHINK BRAIN                        │
  ├──────────────────────────┬───────────────────────────────┤
  │    Existing Modules      │      V15: MetaSystem          │
  │                          │                               │
  │  ┌──── Attention ─────┐  │  ┌──── ErrorMonitor ──────┐   │
  │  │ attention_level     │◄─┼─►│ error_detected → boost │   │
  │  └────────────────────┘  │  │ attention on error      │   │
  │                          │  └─────────────────────────┘   │
  │  ┌──── Predictor ─────┐  │                               │
  │  │ predictive_surprise │◄─┼─►│ confidence.prediction   │   │
  │  └────────────────────┘  │  │ error → meta_record      │   │
  │                          │  │                           │   │
  │  ┌──── Prototype ─────┐  │  ┌──── ConfidenceMonitor ──┐   │
  │  │ classify_confidence │◄─┼─►│ meta_confidence()       │   │
  │  └────────────────────┘  │  └──────────────────────────┘   │
  │                          │                               │
  │  ┌──── Motor ──────────┐  │  ┌──── SelfModel ──────────┐   │
  │  │ motor_confidence    │◄─┼─►│ motor_skill tracking    │   │
  │  └────────────────────┘  │  └──────────────────────────┘   │
  │                          │                               │
  │  ┌──── Social ─────────┐  │  ┌──── StrategySelector ──┐   │
  │  │ prediction_accuracy │◄─┼─►│ select: memory_lookup   │   │
  │  └────────────────────┘  │  │   vs causal_reason      │   │
  │                          │  │   vs pattern_match       │   │
  │                          │  └──────────────────────────┘   │
  └──────────────────────────┴───────────────────────────────┘
...
*Next: [04_build_meta_impl.md](04_build_meta_impl.md) — Full implementation walkthrough*
