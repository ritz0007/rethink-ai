# 🔨 Build: The Rethink Brain — Full Integration

## What We're Building

The `RethinkBrain` struct connects ALL modules from Phases 1-9 into one unified system. One `brain_experience()` call triggers the entire pipeline.

## Code: `src/rethink_brain.h`
...
## How the Code Maps to the Brain

| Code | Brain Region | Function |
|------|-------------|----------|
| `retina` | Primary Visual Cortex (V1) | Edge detection |
| `hopfield` + `decay_mem` | Hippocampus | Memory formation/retrieval |
| `proto` | Inferotemporal Cortex | Object recognition |
| `spontaneous` | Default Mode Network | Internal thought |
| `dream` | Sleep circuits | Memory consolidation |
| `amygdala` | Amygdala | Emotion processing |
| `predictor` | Prefrontal Cortex | Prediction/surprise |
| `causal` | Prefrontal + temporal | Causal reasoning |
| `comm` | Broca/Wernicke areas | Language |
| `attention_level` | Thalamus + TRN | Attention gating |

---

*Next: [Test — A Day in the Life](03_test_full_brain.md)*
